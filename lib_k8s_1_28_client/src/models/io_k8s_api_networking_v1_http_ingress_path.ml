...
type t = {
    backend: Io_k8s_api_networking_v1_ingress_backend.t [@key backend];
    ...
    path_type: string [@key pathType];
} [@@deriving yojson { strict = false }, show ];;

(** HTTPIngressPath associates a path with a backend. Incoming urls matching the path are forwarded to the backend. *)
let create (backend : Io_k8s_api_networking_v1_ingress_backend.t) (path_type : string) : t = {
    backend = backend;
    path = None;
    path_type = path_type;
}
