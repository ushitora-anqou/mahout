...
type t = {
    behavior: Io_k8s_api_autoscaling_v2_horizontal_pod_autoscaler_behavior.t option [@default None] [@key behavior];
    ...
    metrics: Io_k8s_api_autoscaling_v2_metric_spec.t list [@default []] [@key metrics];
    (* minReplicas is the lower limit for the number of replicas to which the autoscaler can scale down.  It defaults to 1 pod.  minReplicas is allowed to be 0 if the alpha feature gate HPAScaleToZero is enabled and at least one Object or External metric is configured.  Scaling is active as long as at least one metric value is available. *)
    min_replicas: int32 option [@default None] [@key minReplicas];
    scale_target_ref: Io_k8s_api_autoscaling_v2_cross_version_object_reference.t [@key scaleTargetRef];
