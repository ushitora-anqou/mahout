...
let create_batch_v1_namespaced_cron_job ~sw client ~namespace ~body ?pretty ?dry_run ?field_manager ?field_validation () =
    ...
    let body = Request.write_as_json_body Io_k8s_api_batch_v1_cron_job.to_yojson body in
...
let create_batch_v1_namespaced_job ~sw client ~namespace ~body ?pretty ?dry_run ?field_manager ?field_validation () =
    ...
    let uri = Request.maybe_add_query_param uri "fieldManager" (fun x -> x) field_manager in
    let uri = Request.maybe_add_query_param uri "fieldValidation" (fun x -> x) field_validation in
    let body = Request.write_as_json_body Io_k8s_api_batch_v1_job.to_yojson body in
    let resp, body = Cohttp_eio.Client.call ~sw client `POST uri ~headers ~body  in
