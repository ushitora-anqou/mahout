...
let create_authentication_v1alpha1_self_subject_review ~sw client ?(headers = Request.default_headers) ~body ?dry_run ?field_manager ?field_validation ?pretty () =
    let uri = Request.build_uri "/apis/authentication.k8s.io/v1alpha1/selfsubjectreviews" in
    ...
    let body = Request.write_as_json_body Io_k8s_api_authentication_v1alpha1_self_subject_review.to_yojson body in
    let resp, body = Cohttp_eio.Client.call ~sw client `POST uri ~headers ~body  in
    Request.read_json_body_as (JsonSupport.unwrap Io_k8s_api_authentication_v1alpha1_self_subject_review.of_yojson) resp body

let get_authentication_v1alpha1_api_resources ~sw client ?(headers = Request.default_headers) () =
    let uri = Request.build_uri "/apis/authentication.k8s.io/v1alpha1/" in
    let headers = Cohttp.Header.add headers "authorization" Request.api_key in
